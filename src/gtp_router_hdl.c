...
gtpc_build_change_notification_response(pkt_buffer_t *pbuff, gtp_session_t *s, gtp_teid_t *teid)
{
	...
	/* Header update */
	gtpc_build_header(pbuff, teid, GTP2C_CHANGE_NOTIFICATION_RESPONSE);
...
gtpc_build_delete_bearer_request(pkt_buffer_t *pbuff, gtp_teid_t *teid, uint8_t cause)
{
	...
	/* Header update */
	gtpc_build_header(pbuff, teid, GTP2C_DELETE_BEARER_REQUEST);
...
gtpc_create_session_request_hdl(gtp_server_worker_t *w, struct sockaddr_storage *addr)
{
	...
	/* IMSI */
	ie_imsi = (gtp_ie_imsi_t*) gtp_msg_ie_get(msg, GTP_IE_IMSI_TYPE);
	if (!ie_imsi) {
...
gtpc_change_notification_request_hdl(gtp_server_worker_t *w, struct sockaddr_storage *addr)
{
	...
	if (!teid) {
		log_message(LOG_INFO, "%s(): Unknown TEID 0x%.8x..."
				    , __FUNCTION__
				    , ntohl(h->teid));
		rc = gtpc_build_errmsg(w->pbuff, NULL, GTP2C_CHANGE_NOTIFICATION_RESPONSE
...
static const struct {
	...
	[GTP2C_BEARER_RESSOURCE_COMMAND]		= { NULL },
	[GTP2C_UPDATE_BEARER_REQUEST]		= { NULL },
	[GTP2C_UPDATE_BEARER_RESPONSE]		= { NULL },
};
