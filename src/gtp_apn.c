...
gtp_apn_hplmn_list_show(vty_t *vty, gtp_apn_t *apn)
{
	...
	pthread_mutex_lock(&apn->mutex);
	list_for_each_entry(hplmn, l, next) {
		char hplmn_s[7];
		plmn_bcd_to_string(hplmn->plmn, hplmn_s);
		vty_out(vty, "%s%s"
			   , hplmn_s
			   , VTY_NEWLINE);
	}
...
apn_service_selection_config_write(vty_t *vty, gtp_apn_t *apn)
{
	...
	list_for_each_entry(service, &apn->service_selection, next) {
		if (!strcmp(service->str,""))
			continue;
		if (service->prio)
			vty_out(vty, " service-selection resolution-service %s prio %d%s"
				   , service->str
				   , service->prio
				   , VTY_NEWLINE);
		else
			vty_out(vty, " service-selection resolution-service %s%s"
				   , service->str
				   , VTY_NEWLINE);
	}
...
DEFUN(apn_service_selection,
      ...
      "service-selection resolution-service STRING prio INTEGER",
